...
export default function WorkoutSession({ workout, onClose }: WorkoutSessionProps) {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => setRestTime(prev => prev - 1), 1000);
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6">
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 200 }} className="text-center">
          ...
          <h1 className="text-5xl font-display gradient-fire-text mb-2">WORKOUT COMPLETE!</h1>
          <p className="text-muted-foreground mb-2">{workout.name} finished</p>
          <p className="text-muted-foreground mb-8">{workout.exercises.length} exercises • ~{workout.caloriesBurned} calories burned</p>
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="flex items-center gap-3 p-4 border-b border-border">
        ...
        <div className="flex-1">
          ...
          <div className="text-sm font-display">Exercise {currentIdx + 1}/{workout.exercises.length}</div>
        </div>
      ...
      <AnimatePresence>
        {resting && (
          ...
          >
            ...
            <h2 className="text-2xl font-display text-muted-foreground mb-2">REST</h2>
      ...
      <div className="px-4 pt-6">
        <AnimatePresence mode="wait">
          <motion.div key={exercise.id} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
            ...
            <div className="bg-card rounded-lg border border-border aspect-video flex items-center justify-center mb-4">
              <div className="text-center">
                <div className="text-4xl mb-2">🎬</div>
                <p className="text-xs text-muted-foreground px-4">Video Demo</p>
              </div>
            </div>
            ...
            <div className="bg-card rounded-lg border border-border p-4 mb-4">
              <div className="grid grid-cols-3 text-center">
                <div><div className="text-2xl font-display">{currentSet}/{exercise.sets}</div><div className="text-[10px] text-muted-foreground uppercase">Set</div></div>
                <div><div className="text-2xl font-display">{exercise.reps}</div><div className="text-[10px] text-muted-foreground uppercase">Reps</div></div>
                <div><div className="text-2xl font-display">{exercise.restSeconds}s</div><div className="text-[10px] text-muted-foreground uppercase">Rest</div></div>
              </div>
            </div>
              ...
              {expanded && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  <div className="bg-card rounded-lg border border-border p-4 mb-2">
                    <h4 className="font-display text-sm mb-2 text-accent">INSTRUCTIONS</h4>
                    <ol className="space-y-1">{exercise.instructions.map((inst, i) => (
                      <li key={i} className="text-sm text-muted-foreground">{i + 1}. {inst}</li>
                    ))}</ol>
                  </div>
                  <div className="bg-card rounded-lg border border-border p-4 mb-2">
                    <h4 className="font-display text-sm mb-2 text-primary">COMMON MISTAKES</h4>
                    <ul className="space-y-1">{exercise.commonMistakes.map((m, i) => (
                      <li key={i} className="text-sm text-muted-foreground">⚠️ {m}</li>
                    ))}</ul>
                  </div>
                  <div className="bg-card rounded-lg border border-border p-4 mb-4">
                    <h4 className="font-display text-sm mb-2 text-warning">MUSCLES WORKED</h4>
                    <div className="flex flex-wrap gap-2">{exercise.musclesWorked.map(m => (
                      <span key={m} className="text-xs px-2 py-1 bg-secondary rounded-full text-muted-foreground">{m}</span>
                    ))}</div>
                  </div>
                </motion.div>
              )}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/95 backdrop-blur border-t border-border">
        ...
        >
          <Check size={20} />
          {currentSet < exercise.sets ? `COMPLETE SET ${currentSet}` : isLast ? 'FINISH WORKOUT' : 'NEXT EXERCISE'}
        </motion.button>
      </div>
    </div>
  );
